...
public struct AiTexture {
    let texturePtr: UnsafePointer<aiTexture>
    private let lifetime: AiSceneLifetime

    var texture: aiTexture { texturePtr.pointee }

    init(texturePtr: UnsafePointer<aiTexture>, lifetime: AiSceneLifetime) {
        self.texturePtr = texturePtr
        self.lifetime = lifetime
        let nativeTexture = texturePtr.pointee

        filename = String(nativeTexture.mFilename)
        achFormatHint = CArray<CChar>.read(nativeTexture.achFormatHint) { body in
            guard let baseAddress = body.baseAddress else {
                return ""
            }
            return String(cString: baseAddress)
        }
        let width = Int(nativeTexture.mWidth)
        self.width = width
        let height = Int(nativeTexture.mHeight)
        self.height = height
        numPixels = {
            if height == 0 {
    ...
    init?(texturePtr aiTexture: UnsafePointer<aiTexture>?, lifetime: AiSceneLifetime) {
        guard let aiTexture else {
            return nil
        }

        self.init(texturePtr: aiTexture, lifetime: lifetime)
    }
    ...
    public mutating func withUnsafeTextureData<R>(_ body: (UnsafeBufferPointer<UInt8>) throws -> R) rethrows -> R {
        guard let dataPtr = texture.pcData else {
            return try body(UnsafeBufferPointer(start: nil, count: 0))
        }
        let count = numPixels * 4 // aiTexel(BGRA) * numPixel
        return try dataPtr.withMemoryRebound(to: UInt8.self, capacity: count) { pBytes in
