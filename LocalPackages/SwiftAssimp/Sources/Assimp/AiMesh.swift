...
public final class AiMesh {
    ...
    let meshPtr: UnsafePointer<aiMesh>
    private let lifetime: AiSceneLifetime

    var mesh: aiMesh { meshPtr.pointee }

    init(meshPtr: UnsafePointer<aiMesh>, lifetime: AiSceneLifetime) {
        self.meshPtr = meshPtr
        self.lifetime = lifetime
        let nativeMesh = meshPtr.pointee
        primitiveTypes = PrimitiveType(rawValue: nativeMesh.mPrimitiveTypes)
        numVertices = Int(nativeMesh.mNumVertices)
        numFaces = Int(nativeMesh.mNumFaces)
        numBones = Int(nativeMesh.mNumBones)
        materialIndex = Int(nativeMesh.mMaterialIndex)
        name = String(nativeMesh.mName)
        numAnimMeshes = Int(nativeMesh.mNumAnimMeshes)
        method = nativeMesh.mMethod.rawValue
    }

    convenience init?(meshPtr: UnsafePointer<aiMesh>?, lifetime: AiSceneLifetime) {
        guard let meshPtr else {
            return nil
        }

        self.init(meshPtr: meshPtr, lifetime: lifetime)
    }
    ...
    public func withUnsafeVertices<R>(_ body: (UnsafeBufferPointer<AiReal>) throws -> R) rethrows -> R {
        guard let verticesPtr = mesh.mVertices else { return try body(UnsafeBufferPointer(start: nil, count: 0)) }
        let count = numVertices * 3
        return try verticesPtr.withMemoryRebound(to: AiReal.self, capacity: count) {
    ...
    public func withUnsafeNormals<R>(_ body: (UnsafeBufferPointer<AiReal>) throws -> R) rethrows -> R {
        guard let normalsPtr = mesh.mNormals else { return try body(UnsafeBufferPointer(start: nil, count: 0)) }
        let count = numVertices * 3
        return try normalsPtr.withMemoryRebound(to: AiReal.self, capacity: count) {
            try body(UnsafeBufferPointer(start: $0, count: count))
        }
    }
    ...
    public func withUnsafeTangents<R>(_ body: (UnsafeBufferPointer<AiReal>) throws -> R) rethrows -> R {
        guard let tangentsPtr = mesh.mTangents else { return try body(UnsafeBufferPointer(start: nil, count: 0)) }
        let count = numVertices * 3
        return try tangentsPtr.withMemoryRebound(to: AiReal.self, capacity: count) {
            try body(UnsafeBufferPointer(start: $0, count: count))
        }
    }
    ...
    public func withUnsafeBitangents<R>(_ body: (UnsafeBufferPointer<AiReal>) throws -> R) rethrows -> R {
        guard let bitangentsPtr = mesh.mBitangents else { return try body(UnsafeBufferPointer(start: nil, count: 0)) }
        let count = numVertices * 3
        return try bitangentsPtr.withMemoryRebound(to: AiReal.self, capacity: count) {
            try body(UnsafeBufferPointer(start: $0, count: count))
        }
    }
    ...
    public lazy var faces: [AiFace] = {
        guard let facesPtr = mesh.mFaces else { return [] }
        return UnsafeBufferPointer(start: facesPtr, count: numFaces).compactMap(AiFace.init)
    }()
    ...
    public struct AiVertexWeight {
        ...
        public let weight: AiReal

        init(_ weight: aiVertexWeight) {
    ...
    public struct AiBone {
        ...
        public let offsetMatrix: AiMatrix4x4
        public let weights: [AiVertexWeight]

        init(_ bone: aiBone) {
            name = String(bone.mName)
            offsetMatrix = AiMatrix4x4(bone.mOffsetMatrix)
            let count = Int(bone.mNumWeights)
            if count > 0, let base = bone.mWeights {
                weights = UnsafeBufferPointer(start: base, count: count).map(AiVertexWeight.init)
            } else {
                weights = []
            }
        }
    ...
    public lazy var bones: [AiBone] = {
        guard numBones > 0, let base = mesh.mBones else { return [] }
        return UnsafeBufferPointer(start: base, count: numBones).compactMap { ptr in
            guard let bone = ptr?.pointee else { return nil }
            return AiBone(bone)
        }
    }()
}
