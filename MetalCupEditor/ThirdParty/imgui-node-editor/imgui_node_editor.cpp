...
#if !defined(IMGUI_VERSION_NUM) || (IMGUI_VERSION_NUM < 18822)
#   define ED_KEY(KEY) ImGui::GetKeyIndex(KEY)
#else
#   define ED_KEY(KEY) (KEY)
#endif
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control& control)
{
    ...
    auto& io = ImGui::GetIO();
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ED_KEY(ImGuiKey_X)))
        candidateAction = Cut;
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ED_KEY(ImGuiKey_C)))
        candidateAction = Copy;
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ED_KEY(ImGuiKey_V)))
        candidateAction = Paste;
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(GetKeyIndexForD()))
        candidateAction = Duplicate;
    if (!io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ED_KEY(ImGuiKey_Space)))
...
ed::EditorAction::AcceptResult ed::DeleteItemsAction::Accept(const Control& control)
{
    ...
    auto& io = ImGui::GetIO();
    if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(ED_KEY(ImGuiKey_Delete)) && Editor->AreShortcutsEnabled())
    {
        auto& selection = Editor->GetSelectedObjects();
        if (!selection.empty())
        {
